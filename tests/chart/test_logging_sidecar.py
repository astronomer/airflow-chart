...
class TestLoggingSidecar:
    ...
    """Test logging sidecar config differences between Airflow 2.x and 3.x"""
    # Test Airflow 2.x
    docs_v2 = render_chart(
        kube_version=kube_version,
        values={
            "loggingSidecar": {"enabled": True},
            "airflow": {
    ...
    vc_v2 = yaml.safe_load(docs_v2[0]["data"]["vector-config.yaml"])

    docs_v3 = render_chart(
        kube_version=kube_version,
        values={
            "loggingSidecar": {"enabled": True},
            "airflow": {
    ...
    vc_v3 = yaml.safe_load(docs_v3[0]["data"]["vector-config.yaml"])

    parse_airflow2_transform = vc_v2["transforms"].get("parse_airflow3_path")
    if parse_airflow2_transform:
        assert "execution_date" in parse_airflow2_transform["source"]
        assert ".is_airflow3 = false" in parse_airflow2_transform["source"]

    parse_airflow3_transform = vc_v3["transforms"]["parse_airflow3_path"]
    assert "parse_json!(.message)" in parse_airflow3_transform["source"]
    assert ".is_airflow3 = true" in parse_airflow3_transform["source"]

    filter_common_logs_v2 = vc_v2["transforms"]["filter_common_logs"]
    filter_common_logs_v3 = vc_v3["transforms"]["filter_common_logs"]

    assert "if exists(.@timestamp)" in filter_common_logs_v3["source"]

    remove_fields_v3 = vc_v3["transforms"]["transform_remove_fields"]
    assert "del(.execution_date)" in remove_fields_v3["source"]
