...
**Bare top-level files.** Single-purpose resources sit directly under `templates/`:

```
templates/ingress.yaml
templates/extra-objects.yaml
templates/airflow-scc-anyuid.yaml
templates/generate-ssl.yaml
templates/logging-sidecar-configmap.yaml
```

In both patterns: lowercase with hyphens, helpers live in `templates/_helpers.yaml`, and the K8s object kind (`deployment`, `configmap`, `networkpolicy`, `service`, `serviceaccount`, `role`, `rolebinding`, `statefulset`, `pvc`, `ingress`, …) is part of the filename so resources are easy to locate. Templates belonging to the bundled Airflow sub-chart live under `charts/airflow/templates/` and follow upstream's conventions — do not restyle them.
